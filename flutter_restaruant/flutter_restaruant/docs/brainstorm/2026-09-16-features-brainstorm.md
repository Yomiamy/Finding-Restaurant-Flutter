...
| **AI 多模態 Vision 菜單翻譯 (F-3.1)** | ✅ **已於 2026-09-12 完成 (Issue #115 / PR #116)**：以 `firebase_ai` + `gemini-3.8-flash` 實現結構化 JSON 菜單辨識與過敏原標註，架構落地於 `lib/domain/`、`lib/data_layer/` 與 `lib/flow/menu_vision/`（34 測試全過零警告，獲 Linus 🟢 好品味與 Production-Ready 裁決；揭露 2 項架構微調 Action Items） |
| **AI 智能覓食助理與 GenUI 決策畫布 (F-3.2 / M4)** | ✅ **已於 2026-09-22 完成 (Issue #123 / PR #124)**：以 `firebase_ai` + `gemini-3.8-flash` 實現自然語言覓食對話、候選店家 Grounding、動態卡片（`comparison_matrix`、`decision_roulette`、`action_chips`）與轉盤隨機挑選抽籤，架構落地於 `lib/domain/`、`lib/data_layer/` 與 `lib/flow/ai_foodie/`；連帶重構多語系 ARB 抽取與認證錯誤解耦（`AuthFailureReason`）。 |
...
* **改造要點**:
  * 參照外部專案之 `release.yml` 流程，為本專案建立專屬的 GitHub Actions CI/CD 工作流。
  * **觸發機制**：依賴 Git Tag 觸發。
  * **開發版 (dev) 發布路線**：
    * **觸發條件**: `dev-ios-{version}`, `dev-android-{version}`
    * **打包與分發**: 打包完成後，上傳至 **Firebase App Distribution** 供內部測試。
  * **正式版 (prd) 發布路線**：
    * **觸發條件**: `prd-ios-{version}`, `prd-android-{version}`
    * **打包與分發**: 打包完成後，上傳至 **Apple Store Connect (TestFlight)** 與 **Google Play Console (Internal Testing)**。
...
|   • [x] S2 共用元件重塑 (ItemCell✅ / RatingStars✅ / Skeleton✅ / Empty✅)           |
...
| **M3** | ✅ 拍菜單多模態視覺助手 (Dish Lens) — 已於 2026-09-12 完成 (PR #116) | 完成相機拍照/選圖、圖片壓縮、`gemini-3.8-flash` 結構化食材/過敏原解析與互動點餐看板 UI | 2.0 |
| **M4** | ✅ AI 智能覓食助理與 GenUI 畫布 — 已於 2026-09-22 完成 (PR #124) | 完成對話式 BottomSheet、真實候選店家 Grounding、`gemini-3.8-flash` 推論、對比卡片與命運轉盤抽籤 | 2.0 |
