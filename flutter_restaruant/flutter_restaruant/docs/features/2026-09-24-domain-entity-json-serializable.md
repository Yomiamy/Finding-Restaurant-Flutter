...
- **v5.5**（PR #127 review 意見）：協定外殼 `{component_type, data}` 的包殼從 5 個子類別的 `toJson` 移到基底類別 `A2UIComponent.toEnvelopeJson()`（sealed 窮盡 switch），與拆殼的分派器 `A2UIComponent.fromJson` 對稱；子類別 `toJson` 改為純產生碼。`AiFoodieMessage.components` 以 `@JsonKey(toJson: _componentsToJson)` 寫出外殼，`ai_foodie_repo.dart` 送回 LLM 的 history 同樣改用 `toEnvelopeJson()`，輸出 JSON 不變。T1 特性測試 `a2ui_characterization_test.dart` 僅把元件的 `.toJson()` 改為 `.toEnvelopeJson()`（輸入與期望值不變）。另 `MenuVisionBloc` 的 `CaptureAndAnalyzeMenu` 例外分支補上 `failedImageBytes`，修正 v5.3 讓解析失敗時失去重試的 regression。
