...
description: 當使用者要更新 Flutter App 的發布版本號時使用。負責建立發布分支，並同步更新 pubspec.yaml 的版號 (version 與 version code) 以及 iOS 專案的 project.pbxproj 中的 MARKETING_VERSION 與 CURRENT_PROJECT_VERSION。觸發語如「bump app 版號」、「更新 app release 版號」、「gen-update-app-release-info 1.4.0 26」。
---

# Gen Update App Release Info

依使用者指定的新版號與 Version Code，建立 release 分支後，同步更新 Flutter 專案中的 App 版本資訊，確保跨平台 (pubspec.yaml 與 iOS pbxproj) 設定一致。
...
- 記當前日期：`$DATE`（`YYYYMM` 格式，如 `202609`），用 `date +%Y%m` 取得。

## 流程

### 1. 建立 Release 分支
在更新任何檔案之前，必須先建立專用的 release 分支，分支命名格式為 `release/$DATE/release-v$VERSION`：
```bash
git checkout -b release/$DATE/release-v$VERSION
```
> **注意**：如果使用者已在預期的 release 分支上則跳過此步驟；若工作區有其他未提交變更，則這些變更會被一併帶入新分支。
...
- 分支已切換至：`release/$DATE/release-v$VERSION`
...
| 步驟 | 檔案/指令 | 改什麼 |
|------|------|--------|
| 1 | `git checkout -b` | 建立並切換至 `release/$DATE/release-v$VERSION` |
| 2 | `pubspec.yaml` | 更新 `version: $VERSION+$VERSION_CODE` |
| 3 | `ios/.../project.pbxproj` | 將所有 `CURRENT_PROJECT_VERSION` 設為 `$VERSION_CODE` |
| 4 | `ios/.../project.pbxproj` | 將所有 `MARKETING_VERSION` 設為 `$VERSION` |
