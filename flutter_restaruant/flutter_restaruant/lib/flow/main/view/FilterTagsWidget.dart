...
class FilterTagsWidget extends StatelessWidget {
  ...
  FilterTagsWidget({Key? key, required filterConfigs})
      : this._filterConfigs = filterConfigs,
        super(key: key) {
    if (this._filterConfigs.sortBy != null &&
        this._filterConfigs.sortBy!.isNotEmpty) {
      this._filterConfigsMap[FilterConfigType.SORTING_RULE] = this
          ._filterConfigs
          .getSortingRuleDispStr(this._filterConfigs.sortBy!);
    }

    if (this._filterConfigs.price != null && this._filterConfigs.price! > 0) {
      this._filterConfigsMap[FilterConfigType.PRICE] =
          this._filterConfigs.getPriceDispStr(this._filterConfigs.price!);
    }

    if (this._filterConfigs.openAt != null && this._filterConfigs.openAt! > 0) {
      this._filterConfigsMap[FilterConfigType.OPEN_AT] =
          this._filterConfigs.openAtDispStr;
    }
  }
              ...
              itemBuilder: (int index) => ItemTags(
                  ...
                  onPressed: (item) {
                    context
                      ...
                      this
                          ._filterConfigs
                          .clearConfig(item.customData as FilterConfigType);
                      mainBloc.add(Reset());
                    });
                  }))));
}
